...
;; Provenance: copied verbatim (modulo the base URL and these pointers) from
;; compile-time-scheme's .emacs.d/lisp/orgit-file-transclusion.el, itself
;; specified by that repo's docs/epistolary-pinning-plan.md (section 3).
;; Shared across sibling repositories by copy, not by load-path coupling, so
;; each repo's copy names its own forge and its own pin table.
;;
...
;; REV is normally an annotated `blog/<post-basename>' tag; see
;; papers/blog/pins.md for the post-to-tag mapping.
;;
...
(defcustom orgit-file-transclusion-base-url
  "https://github.com/steve-downey/transpose/blob/"
