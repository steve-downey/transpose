...
[x+2]{.pnum} *Remarks*: `function` is invoked at most once. Composition accumulates: every error is observed, not only the first. Each operand's held value is passed on with that operand's own value category, so a caller that hands over an rvalue operand has its value moved from rather than copied.
