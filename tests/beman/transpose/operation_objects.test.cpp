...
//
// The operation objects in beman::transpose: one per typeclass
...
// beman::transpose::functor: the only way to reach it is through mode 2. It is
...
struct HeadOnlyFunctorMap
    : bt::derive_functor<HeadOnlyFunctorImpl<VALUE_TYPE>> {
...
// `const auto &TC` NTTP -- the earlier options were to name a lookup
...
TEST_CASE("typeclass: fmap agrees with the lookup object") {
    ...
    const auto &functor = bt::functor<std::vector<int>>;
...
TEST_CASE("typeclass: traverse agrees with the lookup object it replaced") {
    ...
    const auto &traversable = bt::traversable<std::vector<int>>;
...
TEST_CASE("typeclass: fold_map is the primitive and agrees with the object") {
    ...
    const auto &foldable = bt::foldable<std::vector<int>>;
...
TEST_CASE(
    ...
    // `bt::fmap` is an object, so `bt::fmap<...>(f, v)` does not
    ...
    REQUIRE(bt::fmap.operator()<decltype(double_it) &, const std::vector<int> &,
...
TEST_CASE(
    ...
    using pinned =
        bt::pure_fn<std::optional<int>, bt::applicative<std::optional<int>>>;
...
TEST_CASE("typeclass: a missing instance is detectable before the call") {
    ...
    STATIC_REQUIRE_FALSE(
        bt::has_instance_v<decltype(bt::functor<Unregistered>)>);
    STATIC_REQUIRE(bt::has_instance_v<decltype(bt::functor<std::vector<int>>)>);
    STATIC_REQUIRE_FALSE(
        bt::has_instance_v<decltype(bt::foldable<Unregistered>)>);
    STATIC_REQUIRE_FALSE(bt::has_instance_v<decltype(bt::monad<Unregistered>)>);
}
