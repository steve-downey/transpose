...
//
// A fourth lookup mode: the typeclass operation as an object that does its
// own lookup.
//
...
// the instance. This file shows the mode that does not: `bt::fmap(f, tree)`
...
//      NTTP. Before the operation objects, such a function opened with a
//      block of lookup-object declarations, or became a template for no
//      reason of its own.
//
// What the object form costs is mode 3: an operation object takes no explicit
// template arguments, and `bt::fmap<..., my_instance>(f, t)` does not parse.
 ...
 * have been routed through an operation object.
 */
template <class T>
...
template <class T>
struct ReverseFunctorMap : bt::derive_functor<ReverseFunctorImpl<T>> {
...
int main() {
    ...
    // Pinning. bt::fmap takes the registered vector Functor; the instance
