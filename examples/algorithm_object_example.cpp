...
template <class T,
          const auto &TC =
              beman::transpose::traversable_typeclass<beman::transpose::remove_cvref_t<T>>>
struct validate_impl : beman::transpose::remove_cvref_t<decltype(TC)> {
    template <class Pred>
    auto call(Pred &&pred, const T &value) const {
        using element_type =
            typename beman::transpose::remove_cvref_t<decltype(TC)>::element_type;
...
template <class T,
          const auto &FC =
              beman::transpose::foldable_typeclass<beman::transpose::remove_cvref_t<T>>,
          const auto &TC =
              beman::transpose::traversable_typeclass<beman::transpose::remove_cvref_t<T>>>
struct transform_if_large_impl : beman::transpose::remove_cvref_t<decltype(FC)>,
...
struct transform_if_large_fn {
    ...
    auto operator()(std::size_t min_size, F &&f, T &&value) const {
        return detail::transform_if_large_impl<beman::transpose::remove_cvref_t<T>>{}.call(
